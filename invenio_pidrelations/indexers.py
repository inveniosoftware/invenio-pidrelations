...
from .api import PIDVersionRelation
...
def index_relations(sender, json=None, record=None, index=None, **kwargs):
    ...
    if pids:
        for pid in pids:
            ...
            if indexed:
                api = indexed['api']
                head = api.get_head(pid)
                if indexed.get('ordered', False):
                    relation =  dict(
                        parent=head.pid_value,
                        is_latest=api.is_latest(pid),
                        siblings=[pid.pid_value
                                  for pid in api.get_all_versions(pid)],
                        ...
                        # to choose th fields we want to index.
                    )
                else:
                    relation =  dict(
                        parent=head.pid_value,
