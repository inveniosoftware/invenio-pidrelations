...
def test_versioning_children(db, version_pids, build_pid):
    ...
    assert h1.children.ordered("asc").all() == filter_pids(
        version_pids[0]["children"], PIDStatus.REGISTERED
    )
...
def test_versioning_remove_child(db, version_pids, build_pid):
    ...
    assert version_pids[0]["parent"].get_redirect() == version_pids[0]["children"][2]
    ...
    assert version_pids[0]["parent"].get_redirect() == version_pids[0]["children"][1]
...
def test_versioning_insert_draft_child(db, version_pids, build_pid):
    ...
    draft2 = PersistentIdentifier.create(
        "recid", "foobar.draft2", object_type="rec", status=PIDStatus.RESERVED
    )
    with pytest.raises(PIDRelationConsistencyError):
...
def test_update_redirect(db, version_pids, build_pid):
    ...
    def test_redirect(expected_length, expected_redirect):
        filtered = filter_pids(version_pids[0]["children"], status=PIDStatus.REGISTERED)
    ...
    for pid in filter_pids(version_pids[0]["children"], status=PIDStatus.REGISTERED):
        pid.delete()
    h1.update_redirect()
