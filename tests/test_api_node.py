...
def test_node_children(db, version_relation, version_pids, build_pid):
    ...
    parent_node = PIDNode(build_pid(version_pids[0]["parent"]), version_relation)
    assert parent_node.children.ordered("asc").all() == version_pids[0]["children"]

    # Check that status filtering works on children
    assert parent_node.children.status(PIDStatus.REGISTERED).ordered(
        "asc"
    ).all() == filter_pids(version_pids[0]["children"], PIDStatus.REGISTERED)

    # Test children of a PID having no children
    child_node = PIDNode(build_pid(version_pids[0]["children"][0]), version_relation)
...
@with_pid_and_fetched_pid
def test_node_insert_child(db, version_relation, version_pids, build_pid, recids):
    """Test PIDNode.insert_child."""
    parent_pid = build_pid(version_pids[0]["parent"])
    parent_node = PIDNode(parent_pid, version_relation)
    child_pid = build_pid(recids[str(PIDStatus.REGISTERED)])
    child_node = PIDNode(child_pid, version_relation)
...
@with_pid_and_fetched_pid
def test_node_remove_child(db, version_relation, version_pids, build_pid, recids):
...
@with_pid_and_fetched_pid
def test_ordered_node_index(db, version_relation, version_pids, build_pid, recids):
    ...
    assert ordered_parent_node.index(child_pid) == len(version_pids[0]["children"])


@with_pid_and_fetched_pid
def test_ordered_node_last_child(db, version_relation, version_pids, build_pid, recids):
...
@with_pid_and_fetched_pid
def test_ordered_node_next_child(db, version_relation, version_pids, build_pid, recids):
    """Test the PIDNodeOrdered next_child method."""
    parent_pid = build_pid(version_pids[0]["parent"])
    ordered_parent_node = PIDNodeOrdered(parent_pid, version_relation)
    assert (
        ordered_parent_node.next_child(version_pids[0]["children"][0])
        == version_pids[0]["children"][1]
    )
    # Check that the next child can be retrieved if there is a "hole" in the
    # sequence of indices.
    ordered_parent_node.remove_child(version_pids[0]["children"][1], reorder=False)
    ...
    assert ordered_parent_node.next_child(version_pids[0]["children"][-1]) is None


@with_pid_and_fetched_pid
def test_ordered_node_previous_child(
    db, version_relation, version_pids, build_pid, recids
):
    """Test the PIDNodeOrdered previous_child method."""
    parent_pid = build_pid(version_pids[0]["parent"])
    ordered_parent_node = PIDNodeOrdered(parent_pid, version_relation)
    assert (
        ordered_parent_node.previous_child(version_pids[0]["children"][-1])
        == version_pids[0]["children"][-2]
    )
    # Check that the previous child can be retrieved if there is a "hole" in
    # the sequence of indices.
    ordered_parent_node.remove_child(version_pids[0]["children"][-2], reorder=False)
    ...
    assert ordered_parent_node.previous_child(version_pids[0]["children"][0]) is None


@with_pid_and_fetched_pid
def test_ordered_node_is_last_child(
    db, version_relation, version_pids, build_pid, recids
):
    """Test the PIDNodeOrdered is_last_child method."""
    parent_pid = build_pid(version_pids[0]["parent"])
    ordered_parent_node = PIDNodeOrdered(parent_pid, version_relation)
    assert ordered_parent_node.is_last_child(version_pids[0]["children"][-1])
...
@with_pid_and_fetched_pid
def test_ordered_node_insert(db, version_relation, version_pids, build_pid, recids):
...
@with_pid_and_fetched_pid
def test_ordered_node_remove_with_reorder(
    db, version_relation, version_pids, build_pid, recids
):
    """Test PIDNode.remove_child."""
    parent_pid = build_pid(version_pids[0]["parent"])
    ...
    ordered_parent_node.remove_child(version_pids[0]["children"][0], reorder=True)
    del version_pids[0]["children"][0]
    assert_children_indices(ordered_parent_node, version_pids[0]["children"])

    # c-c-c-c-x
    # remove the last child
    ordered_parent_node.remove_child(version_pids[0]["children"][-1], reorder=True)
    del version_pids[0]["children"][-1]
    assert_children_indices(ordered_parent_node, version_pids[0]["children"])
...
@with_pid_and_fetched_pid
def test_ordered_node_remove_without_reorder(
    db, version_relation, version_pids, build_pid, recids
):
    """Test PIDNode.remove_child."""
    parent_pid = build_pid(version_pids[0]["parent"])
    ...
    # x-c-c-c-c
    # remove the first child
    ordered_parent_node.remove_child(version_pids[0]["children"][0], reorder=False)
    del version_pids[0]["children"][0]
    assert len(ordered_parent_node.children.all()) == len(version_pids[0]["children"])
    for idx, child_pid in enumerate(version_pids[0]["children"]):
...
@with_pid_and_fetched_pid
def test_node_max_parents(db, version_relation, version_pids, build_pid, recids):
    ...
    ordered_parent_node_1 = PIDNode(parent_pid_1, version_relation, max_parents=1)
    ordered_parent_node_2 = PIDNode(parent_pid_2, version_relation, max_parents=1)
...
@with_pid_and_fetched_pid
def test_node_max_children(db, version_relation, version_pids, build_pid, recids):
    """Test the PIDNode max children attribute."""
    parent_pid = build_pid(version_pids[0]["parent"])
    ordered_parent_node = PIDNode(
        parent_pid, version_relation, max_children=len(version_pids[0]["children"])
    )
    child_pids = create_pids(1)
